...
app.post('/api/query', async (req, res) => {
    try {
        ...
        const htmlPath = path.join(__dirname, 'public', 'cases', `${selectedPatient}.md`);
        ...
        const response = await axios.post('https://api.openai.com/v1/chat/completions', {
            model: "gpt-4",
            messages: [
                { role: "system", content: `${characterDescription} You are involved in the following case study: ${caseStudy}` },
        ...
        req.session.chatHistory.push({ role: "user", content: req.body.prompt });
        req.session.chatHistory.push({ role: "assistant", content: messageContent });

        res.json({ message: messageContent });

    } catch (error) {
        console.error('Error with OpenAI API:', error?.response?.data || error.message);
        res.status(500).json({ error: 'Failed to fetch response from OpenAI', details: error?.response?.data || error.message });
    }
});
...
app.post('/api/select-patient', async (req, res) => {
    ...
    try {
        ...
        req.session.chatHistory = [{ role: 'system', content: `Patient selected: ${selectedPatient}` }];
